...
d3.csv("assets/data/covid19.csv", function (covid19) {
  ...
  // create a time array ==================
  var datesList = covid19.map(function (d) { return d.date }).filter(function (value, index, self) {
    return self.indexOf(value) === index;
  });
  var datesMMDD = []
  for (var i = 0; i < datesList.length - 1; i++) {
    datesMMDD.push(datesList[i].split("-")[1] + "-" + datesList[i].split("-")[2])
  };
  // =======================================
...
d3.csv('https://cors-anywhere.herokuapp.com/https://github.com/CSSEGISandData/COVID-19/blob/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv', data => {
  githubData = data;
});